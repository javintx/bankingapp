...
class UserControllerTest {
  ...
  void loginUser_SuccessfulLogin() throws Exception {
    var identifier = "nuwe@nuwe.com";
    var password = "NuweTest1$";
    User user = new User("Nuwe Test", identifier, password,
        "$2a$10$vYWBxACqEIPeoT0O5b0faOHp4ITAHSBvoHDzBePW7tPqzpvqKLi6G", emptyList());

    when(userService.findByEmail(identifier)).thenReturn(Optional.of(user));
    when(bCryptPasswordEncoder.matches(password, user.hashedPassword())).thenReturn(true);
    ...
    verify(userService, times(1)).findByEmail(identifier);
    verify(bCryptPasswordEncoder, times(1)).matches(password, user.hashedPassword());
  ...
  void loginUser_BadCredentials() throws Exception {
    ...
    User user = new User("Nuwe Test", identifier, "NuweTest1$",
        "$2a$10$vYWBxACqEIPeoT0O5b0faOHp4ITAHSBvoHDzBePW7tPqzpvqKLi6G", emptyList());

    when(userService.findByEmail(identifier)).thenReturn(Optional.of(user));
    when(bCryptPasswordEncoder.matches(password, user.hashedPassword())).thenReturn(false);
  ...
  void loginUser_UserNotFound() throws Exception {
    var identifier = "unknown@nuwe.com";
    var password = "NuweTest1$";
    when(userService.findByEmail(identifier)).thenReturn(Optional.empty());
